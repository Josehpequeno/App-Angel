        ...
        <p>
			...
			<div id="body">
				...
				<div id="posts">
					...
					<div id="post">
						<?php 
							$user = $_COOKIE["user"];
							$chet = mysqli_query($conn,"SELECT id FROM usuario WHERE email = '$user'");
							$feth = mysqli_fetch_row($chet);
							$id_user = $feth[0];
							$chek = mysqli_query($conn,"SELECT id_arquivo FROM salvos WHERE id_user = '$id_user'");
							if (mysqli_num_rows($chek) == 0) {
									echo "<h3> Você não tem arquivos salvos no momento.</h3>";
							}else{
								while ($fety = mysqli_fetch_row($chek)) {

									$id_arq = $fety[0];
									echo "<h2> Lista de arquivos Salvos </h2>";
									$check = mysqli_query($conn,"SELECT user,texto,imagem,data,cod_arquivos,id FROM arquivos WHERE id = '$id_arq'");
									if(mysqli_num_rows($check) == 0){
										echo "<h3> temos algum erro  nesse  momento.</h3>";
									}else{
										while($fetch = mysqli_fetch_row($check)){
		    							?>
		    							<form method="POST" id="salvar" action=<?php echo "processos/trata_salvar.php?d=".$fetch[5]; ?>>
				    						<table width="380px">
				    							<tr><td>
													...
													<input type="submit" id="submit" value="Retirar da lista de salvos"/>
												</td></tr>
											</table>
										</form>
	    								<?php
			...
			<script type="text/javascript">
                ...
                $('#deletar').submit(function(e){
	                ...
	                $.ajax({
	                    url: <?php echo "processos/trata_salvar.php?f=$fetch[5]"; ?>,
	                    type: "POST",
	                    data: formData,
	                    async: true,
	                    cache : false,
