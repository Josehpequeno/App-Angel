			...
			<div id="body">
				...
				<div id="posts">
					<form method="POST" id="publicar" >
						<a href="Configuracoes.php">Alterar Dados </a>
					</form>
					<div id="post">
						<?php 
							$email = $_COOKIE["user"];
							$check = mysqli_query($conn,"SELECT nome,apelido,email,biografia FROM usuario WHERE email='$email' LIMIT 1 ");
							if(mysqli_num_rows($check) == 0){
								...
								<!--<tr><th><?php// echo $fetch[0]; ?></th>
								<th><?php// echo $fetch[1]; ?></th><a href="#"></a>></tr>-->
    							<?php
							}	
						?>
					</div>
					<div id="post">
						<form method="POST" id="cadastro" class="log" action="Lista_Arquivos_Salvos.php">
				            <input type="submit" value="Lista de arquivos salvos" name="criar" class="btn"/>
			        	</form>
					</div>
				</div>
			</div>
